...
int main() {
    ...
    DEFAULT_LOGGER_RAW()->setFormatter("[%Y-%m-%d %H:%M:%S.%e] [%l] [thread %t] [%@]: %v");
    FOXZT_LOGGER_CALL(DEFAULT_LOGGER_RAW(), foxzt::LogLevel::WARN, "hello {}", "world");
    FOXZT_LOGGER_CALL(DEFAULT_LOGGER_RAW(), foxzt::LogLevel::WARN, "hello");

    auto logger = LOGGER_NAME_RAW("root");
    logger->setFormatter("[%Y-%m-%d %H:%M:%S.%e] [%l] [thread %t] [%n] [%@]: %v");
    FOXZT_LOGGER_INFO(logger, "asd{}", 123);
